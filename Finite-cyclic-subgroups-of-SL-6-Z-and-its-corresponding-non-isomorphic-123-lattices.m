...
lattices6:=[]; for i in [1..85308] do if IsCyclic(Group(cryst6[i]))=true and Determinant(GeneratorsOfGroup(Group(cryst6[i]))[1])=1 then Append(lattices6,[cryst6[i]]); fi; od;
...
/* One can check that there are 123 subgroups computing */ Size(lattices6);
...
F:=FreeGroup("a","b","c","d","e","f","t"); AssignGeneratorVariables(F); A:=[]; for i in [1..123] do A[i]:=lattices6[i][1]^-1;od; G:=[]; for i in [1..123] do G[i]:=F/[Comm(a,b),Comm(a,c),Comm(a,d),Comm(a,e),Comm(a,f),Comm(b,c),Comm(b,d),Comm(b,e),Comm(b,f),Comm(c,d),Comm(c,e),Comm(c,f),Comm(d,e),Comm(d,f),Comm(e,f), a^t*(a^A[i][1,1]*b^A[i][2,1]*c^A[i][3,1]*d^A[i][4,1]*e^A[i][5,1]f^A[i][6,1])^-1, b^t(a^A[i][1,2]*b^A[i][2,2]*c^A[i][3,2]*d^A[i][4,2]*e^A[i][5,2]f^A[i][6,2])^-1, c^t(a^A[i][1,3]*b^A[i][2,3]*c^A[i][3,3]*d^A[i][4,3]*e^A[i][5,3]f^A[i][6,3])^-1, d^t(a^A[i][1,4]*b^A[i][2,4]*c^A[i][3,4]*d^A[i][4,4]*e^A[i][5,4]f^A[i][6,4])^-1, e^t(a^A[i][1,5]*b^A[i][2,5]*c^A[i][3,5]*d^A[i][4,5]*e^A[i][5,5]f^A[i][6,5])^-1,
...
indexes:=[]; for i in [1..123] do indexes[i]:=Size(LowIndexSubgroupsFpGroup(G[i],7));od; for i in [1..123] do for j in [(i+1)..123] do if indexes[i]=indexes[j] then Print([i,j,Size(LowIndexSubgroupsFpGroup(G[i],8)),Size(LowIndexSubgroupsFpGroup(G[j],8))], " ");fi;od;od;

/* Obtaining the following: */ [ 12, 31, 532, 284 ],[ 26, 34, 676, 702 ],[ 29, 44, 101, 84 ],[ 35, 36, 486, 396 ], [ 37, 58, 570, 1054 ], [ 37, 64, 570, 1054 ], [ 55, 61, 3346, 3346 ], [ 57, 63, 1658, 1658 ], [ 58, 64, 1054, 1054 ], [ 78, 86, 42, 47 ], [ 82, 88, 22, 24 ], [ 89, 122, 43, 54 ],[ 103, 105, 5180, 6428 ], [ 106, 107, 4380, 4380 ], [ 109, 110, 3076, 4324 ],[ 111, 113, 2532, 2532 ]

/* As we see, there are many groups that are distinguished by the number of subgroups with index \leq 8. We still have to distinguish between: G[55] and G[61], G[57] and G[63], G[58] and G[64], G[106] and G[107], G[111] and G[113]. */

Size(LowIndexSubgroupsFpGroup(G[55],9))-Size(LowIndexSubgroupsFpGroup(G[61],9)); /* is different from zero */

Size(LowIndexSubgroupsFpGroup(G[57],9))-Size(LowIndexSubgroupsFpGroup(G[63],9)); /* is different from zero */

Size(LowIndexSubgroupsFpGroup(G[58],9))-Size(LowIndexSubgroupsFpGroup(G[64],9)); /* is different from zero */
